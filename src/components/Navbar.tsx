...
export function StickyHeader({
  ...
  return (
    <header
      ref={stickyNavRef}
      className={`fixed top-4 left-4 right-4 z-50 bg-black/95 backdrop-blur-sm rounded-full mx-auto max-w-6xl transition-all duration-300 ${
        scrollY >= 120 ? "px-4 py-2" : "px-6 py-4"
      }`}
    >
      <nav className="relative mx-auto flex items-center justify-between max-w-6xl">
        <div className="flex items-center gap-2">
          <motion.img
            className={`transition-all duration-300 ${
              scrollY >= 120 ? "h-8 w-auto" : "h-12 w-auto"
            }`}
            src="/Trivano.png"
            alt="Company Logo"
            animate={{
        ...
        <motion.div
          ...
          transition={{ duration: 0.15 }}
        >
          <button className="bg-[#BAFF38] text-zinc-900- text-md   px-6 py-2 rounded-full border border-gray-600 transition-colors">
            Get Started
          </button>
        </motion.div>
